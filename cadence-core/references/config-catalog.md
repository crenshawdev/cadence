...
| **Model** |||||
| `roles.cad-planner.model` | str\|null | Which model turns a phase's context into the plan files every executor runs from? It runs once or twice a phase, and a weak plan is paid for again by every dispatch that reads it | `opus`→strongest, most expensive per dispatch · `sonnet`→the middle; most planning lands here · `haiku`→cheapest and weakest · empty→`null`, send no model parameter and run at your session's model. `fable` is accepted too, reachable via `Other`: choosing it asserts something about your own organisation (30-day data retention, and classifiers that refuse cyber-adjacent review work) | null |
| `roles.cad-planner.effort` | enum | How hard should the planner think, at the rung it STARTS from? | `low` · `medium` · `high` · `xhigh` · `max` · empty→`null`, this key pins nothing and the default decides. Over the four-option cap, so show the rung in force plus three and name the rest reachable via `Other` | high |
| `roles.cad-assumptions-analyzer.model` | str\|null | Which model reads a phase's context before any plan exists and names what the work is assuming? Once a phase, and the cheapest place there is to catch a wrong premise | same four as the planner, plus `fable` via `Other` | null |
| `roles.cad-assumptions-analyzer.effort` | enum | How hard should the assumptions analyzer think, at the rung it STARTS from? | `low` · `medium` · `high` · `xhigh` · `max` · empty→`null` (the default decides); over the cap, three plus `Other` | high |
| `roles.cad-verifier.model` | str\|null | Which model runs the goal-backward pass after a phase - asking whether the goal is met, not whether the tasks ran? Only when the deep pass is on | same four as the planner, plus `fable` via `Other` | null |
| `roles.cad-verifier.effort` | enum | How hard should the verifier think, at the rung it STARTS from? | `low` · `medium` · `high` · `xhigh` · `max` · empty→`null` (the default decides); over the cap, three plus `Other` | high |
| `roles.cad-reviewer.model` | str\|null | Which model runs the local reviewer behind every review trigger? The most-dispatched role after the executor, so this moves a cycle's total cost more per step than any other answer here | same four as the planner, plus `fable` via `Other` | null |
| `roles.cad-reviewer.effort` | enum | How hard should the local reviewer think, at the rung it STARTS from? | `low` · `medium` · `high` · `xhigh` · `max` · empty→`null` (the default decides); over the cap, three plus `Other` | medium |
| `roles.cad-executor.model` | str\|null | Which model writes the code, one dispatch per plan? The single largest spend in a phase | same four as the planner, plus `fable` via `Other` | null |
| `roles.cad-executor.effort` | enum | How hard should the executor think, at the rung it STARTS from? | `low` · `medium` · `high` · `xhigh` · `max` · empty→`null` (the default decides); over the cap, three plus `Other` | high |
| `roles.cad-plan-checker.model` | str\|null | Which model checks a plan against its context before any code is written? Only when `workflow.plan_check` is on - short, structured work | same four as the planner, plus `fable` via `Other` | null |
| `roles.cad-plan-checker.effort` | enum | How hard should the plan checker think, at the rung it STARTS from? | `low` · `medium` · `high` · `xhigh` · `max` · empty→`null` (the default decides); over the cap, three plus `Other` | low |
| `model.escalate_on_failure` | bool | Re-dispatch a failed attempt one rung above the rung it started at | `true`→retry one rung up, holding at the top rung · `false`→hold the retry at the rung it started on | false |
| **Workflow** |||||
...
| `workflow.verifier` | bool | Off switch for goal-backward verification after a phase | `true`→the deep pass runs when the risk floor raises, i.e. when the phase's plans declare a file touching an answered risk surface (`--deep` forces it either way) · `false`→always skip | true |
...
| `workflow.max_plan_bytes` | int | Byte ceiling PER PLAN on what its `files:` frontmatter declares, summed over the on-disk sizes of those paths - the read set an executor dispatch is handed. Measured at `check_size` by `planning.mjs plan-size --max-bytes`, which reports the crossing and refuses nothing; the remedy is more plans, as it is for `workflow.max_plan_tasks`. The default is the 75th percentile of the declared-byte distribution across this repository's 43 archived plans rounded up to the next 25,000 (p75 667,580, max 1,282,083, measured 2026-08-26; it fires on 10 of the 43), the derivation rule the six `workflow.max_dispatch_tokens.*` keys already state | e.g. `675000` | 675000 |
...
| `git.forge_repo` `[repo-layer-only]` | str\|null | Which repository on that forge - the `owner/name` slug every forge call addresses | `owner/name`, two or more segments and a nested GitLab subgroup path allowed, shape-checked when you set it · empty→`null` (not asked yet) | null |
| `git.forge_host` | str\|null | Which Forgejo instance serves it - the host you reach in a browser, never read off the remote URL | `hostname`, or `hostname:port` when the instance is not on the default port - the port your BROWSER reaches, never the SSH one, `1`-`65535` with no leading zero, shape-checked when you set it · empty→`null` (not asked yet, and always null on `github` and `gitlab`) | null |
...
| `review.triggers.<t>.gate` `[repo]` | enum | When this review runs, what should happen to what it finds? | `off`→do not run it · `advisory`→run it and just report what it finds · `deferred`→run it, save the findings, and let the work continue - `/cad-land` refuses to publish until you rule on them · `blocking`→run it and stop the work until the findings are dealt with · `adjudicated`→run it, check each finding against the code, then show you the ones that hold and ask which to act on (nothing is preselected) | per trigger: `plan` advisory (raised to blocking by the risk floor), `diff` off, `risk_surface` blocking, `phase_diff` off |
| `review.triggers.<t>.tier` `[repo]` | enum | Model tier for this trigger - **cross-model only** (the claude-subagent reviewer's model is the one its own role resolves) | `flagship` · `balanced` · `cheap` | cheap, on all four triggers |
| `review.triggers.<t>.effort` `[repo]` | enum | Reasoning effort for this trigger - **cross-model only** (claude-subagent effort is frontmatter-frozen) | `minimal` · `low` · `medium` · `high` | per trigger: `plan` low, `diff` minimal, `risk_surface` low, `phase_diff` low |
...
| `review.triggers.risk_surface.waive_routing_floor` `[repo]` | list(enum) | Which risky-code categories should stop raising the bar on a phase that touches them? (risk_surface only) - a category picked here stops making the plan review blocking and stops turning the deep-verify pass on; it moves no role's model and no role's rung, and it cannot turn the `risk_surface` review off | `auth`, `migrations`, `billing`, `concurrency`, `destructive`, `secrets`, `api_contract`, `untrusted_input` - the same eight as `surfaces`, listed because this row is where a reader meets them. A category you leave out still raises both. Every category actually waived is named in the routing decision's `reason` | unset→nothing is waived |
