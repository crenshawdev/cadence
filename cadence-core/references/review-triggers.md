...
```
node "${CLAUDE_PLUGIN_ROOT}/cadence-core/bin/planning.mjs" trace append --phase <N> --family outcome --event adjudication --trigger <trigger> --sha <head> --raised <findings the reviewers raised before adjudication> --detail-file <path>
```
...
`--sha <head>` names the RANGE this adjudication settled, and it is what lets
