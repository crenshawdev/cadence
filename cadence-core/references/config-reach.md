...
| `model.overrides.cad-planner` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-planner.model`: it names this role's model only when the roles key names none, and setting both warns and resolves the roles key |
| `model.overrides.cad-assumptions-analyzer` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-assumptions-analyzer.model`: it names this role's model only when the roles key names none, and setting both warns and resolves the roles key |
| `model.overrides.cad-verifier` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-verifier.model`: it names this role's model only when the roles key names none, and setting both warns and resolves the roles key |
| `model.overrides.cad-reviewer` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-reviewer.model`: it names this role's model only when the roles key names none, and setting both warns and resolves the roles key |
| `model.overrides.cad-executor` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-executor.model`: it names this role's model only when the roles key names none, and setting both warns and resolves the roles key |
| `model.overrides.cad-plan-checker` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-plan-checker.model`: it names this role's model only when the roles key names none, and setting both warns and resolves the roles key |
| `model.effort.cad-planner` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-planner.effort`: it names the rung this role starts at only when the roles key names none; no floor reaches it (pre-plan role, dispatched before the PLAN the floor reads) |
| `model.effort.cad-assumptions-analyzer` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-assumptions-analyzer.effort`: it names the rung this role starts at only when the roles key names none; no floor reaches it (pre-plan role, dispatched before the PLAN the floor reads) |
| `model.effort.cad-verifier` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-verifier.effort`: it names the rung this role starts at only when the roles key names none, and no risk floor moves it |
| `model.effort.cad-reviewer` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-reviewer.effort`: it names the rung this role starts at only when the roles key names none, and no risk floor moves it |
| `model.effort.cad-executor` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-executor.effort`: it names the rung this role starts at only when the roles key names none, and no risk floor moves it |
| `model.effort.cad-plan-checker` | universal | `bin/route.mjs` - the narrower FALLBACK under `roles.cad-plan-checker.effort`: it names the rung this role starts at only when the roles key names none, and no risk floor moves it |
| `roles.cad-planner.model` | universal | `bin/route.mjs` - names this role's model outright and is the FIRST source, winning over `model.overrides.cad-planner`; unset sends no model parameter, and a string the host does not accept warns, sends no model parameter and never falls through to the older key |
| `roles.cad-assumptions-analyzer.model` | universal | `bin/route.mjs` - names this role's model outright and is the FIRST source, winning over `model.overrides.cad-assumptions-analyzer`; unset sends no model parameter, and a string the host does not accept warns, sends no model parameter and never falls through to the older key |
| `roles.cad-verifier.model` | universal | `bin/route.mjs` - names this role's model outright and is the FIRST source, winning over `model.overrides.cad-verifier`; unset sends no model parameter, and a string the host does not accept warns, sends no model parameter and never falls through to the older key |
| `roles.cad-reviewer.model` | universal | `bin/route.mjs` - names this role's model outright and is the FIRST source, winning over `model.overrides.cad-reviewer`; unset sends no model parameter, and a string the host does not accept warns, sends no model parameter and never falls through to the older key |
| `roles.cad-executor.model` | universal | `bin/route.mjs` - names this role's model outright and is the FIRST source, winning over `model.overrides.cad-executor`; unset sends no model parameter, and a string the host does not accept warns, sends no model parameter and never falls through to the older key |
| `roles.cad-plan-checker.model` | universal | `bin/route.mjs` - names this role's model outright and is the FIRST source, winning over `model.overrides.cad-plan-checker`; unset sends no model parameter, and a string the host does not accept warns, sends no model parameter and never falls through to the older key |
| `roles.cad-planner.effort` | universal | `bin/route.mjs` - names the rung this role starts at and is the FIRST source, winning over `model.effort.cad-planner`; unset takes this key's own schema default, and no floor reaches it (pre-plan role, dispatched before the PLAN the floor reads) |
| `roles.cad-assumptions-analyzer.effort` | universal | `bin/route.mjs` - names the rung this role starts at and is the FIRST source, winning over `model.effort.cad-assumptions-analyzer`; unset takes this key's own schema default, and no floor reaches it (pre-plan role, dispatched before the PLAN the floor reads) |
| `roles.cad-verifier.effort` | universal | `bin/route.mjs` - names the rung this role starts at and is the FIRST source, winning over `model.effort.cad-verifier`; unset takes this key's own schema default, and no risk floor moves it |
| `roles.cad-reviewer.effort` | universal | `bin/route.mjs` - names the rung this role starts at and is the FIRST source, winning over `model.effort.cad-reviewer`; unset takes this key's own schema default, and no risk floor moves it |
| `roles.cad-executor.effort` | universal | `bin/route.mjs` - names the rung this role starts at and is the FIRST source, winning over `model.effort.cad-executor`; unset takes this key's own schema default, and no risk floor moves it |
| `roles.cad-plan-checker.effort` | universal | `bin/route.mjs` - names the rung this role starts at and is the FIRST source, winning over `model.effort.cad-plan-checker`; unset takes this key's own schema default, and no risk floor moves it |
...
| `review.triggers.plan.gate` | universal | `bin/route.mjs resolve` - the plan review's gate, over the default |
| `review.triggers.plan.tier` | cross-model reviewers only | `bin/route.mjs resolve` - the cross-model tier, resolved per trigger into the `reviewer_tiers` map beside `reviewers` (review-triggers.md step 4 TAKES it and indexes `review.providers.<name>.tiers` with it); a layer that sets it wins, this key's own schema default answers otherwise, and the claude-subagent reviewer's model is the one `cad-reviewer` resolves |
| `review.triggers.plan.effort` | cross-model reviewers only | `bin/review-provider.mjs` `--effort`, resolved per trigger by `bin/route.mjs resolve` into the `reviewer_efforts` map beside `reviewers` (review-triggers.md step 4 TAKES it); a layer that sets it wins, this key's own schema default answers otherwise, and the claude-subagent reviewer's effort is frontmatter-frozen |
| `review.triggers.diff.gate` | universal | `bin/route.mjs resolve` - the diff review's gate, over the default |
| `review.triggers.diff.tier` | cross-model reviewers only | `bin/route.mjs resolve` - the cross-model tier, resolved per trigger into the `reviewer_tiers` map beside `reviewers` (review-triggers.md step 4 TAKES it and indexes `review.providers.<name>.tiers` with it); a layer that sets it wins, this key's own schema default answers otherwise, and the claude-subagent reviewer's model is the one `cad-reviewer` resolves |
| `review.triggers.diff.effort` | cross-model reviewers only | `bin/review-provider.mjs` `--effort`, resolved per trigger by `bin/route.mjs resolve` into the `reviewer_efforts` map beside `reviewers` (review-triggers.md step 4 TAKES it); a layer that sets it wins, this key's own schema default answers otherwise, and the claude-subagent reviewer's effort is frontmatter-frozen |
| `review.triggers.risk_surface.gate` | universal | `bin/route.mjs resolve` - the risk-surface review's gate, over the default |
...
| `review.triggers.risk_surface.waive_routing_floor` | universal | `bin/route.mjs resolve` - the surfaces whose plan-time RAISE is waived: a surface named here stops making the plan review blocking and stops turning the deep-verify pass on, which are the floor's only two effects, and it moves no role's model or rung either way. Every waiver applied is named in `reason`, and it waives no review - the blocking commit-time `risk_surface` review still fires on the diff |
| `review.triggers.phase_diff.gate` | universal | `bin/route.mjs resolve` - the merged-phase review's gate, over the default |
