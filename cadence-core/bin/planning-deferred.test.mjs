// Zero-dep tests for `planning.mjs deferred record / list - the queued gate findings`. Run:
...
// --- a historical deferred home ------------------------------------------

/** Write a queue member straight into its selected home, no repository needed. */
...
test('adjudication + deferred record: a carried fire resolves its home in deferred/<N>/', () => {
  // Historical deferred homes remain readable and writable after their phase
  // directory is gone. Build that home directly without a carry writer.
  const { repo, dir, base } = adjRepo({ phase: 4 });
  mkdirSync(join(dir, 'deferred', '4'), { recursive: true });
  putMember(dir, 4, 'diff', 'plan-1', 1, 1, 'deferred');
