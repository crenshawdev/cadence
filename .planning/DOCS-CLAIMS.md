...
| EXECUTE-22 | cadence-core/bin/planning/trace.mjs | 58-78 | `.planning/trace.jsonl` is gitignored; `/cad-new-project` writes the line via `planning.mjs trace ignore` and `/cad-health` only reports a pre-seam scaffold. | accurate | accurate | 2 |
...
| REPORT-12 | cadence-core/workflows/report.md | 170-176 | Cross-model provider spend is NOT on the token line's exclusion list: the seam records the provider's own reported usage on the `provider/request` event and the `Cross-model reviews` line reports it in its own denomination, separate from this total rather than missing from it | accurate | corrected - TRC-12/CST-04, v3.7.10 phase 1 (plan 3 task 4): the claim was true when run 1 read it and stopped being true when the seam started extracting provider usage. `SPEND_EXCLUDES` dropped `cross-model provider calls` and `report.md` now states where that spend IS reported. Claim text rewritten to the live sentence and the anchor re-pinned to where the live file states it, the README-32 shape, so the row still joins; the run-1 verdict cell is left exactly as run 1 recorded it | 2 |
| REPORT-13 | cadence-core/workflows/report.md | 233-244 | The coordinator residue is `coordinator.residue_ms` and the `steps[]` row carrying the most of it, scoped to one RUN by `corr` rather than to a phase - each run's last marker closes at that run's own last event and no window spans two runs | accurate | accurate | 2 |
| REPORT-14 | cadence-core/workflows/report.md | 234-235 | The renderer computes it once so this line and `trace suggest` cannot disagree | accurate | accurate | 2 |
| REPORT-15 | cadence-core/workflows/report.md | 245-247 | Residue is TIME between worker brackets, never tokens; a marker carries no token figure | accurate | accurate | 2 |
