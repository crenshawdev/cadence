...
| MILESTONE-06 | cadence-core/workflows/milestone.md | 67-71 | An `ok:false` with `action:"refuse"` names one of `no-target-version`, `unparseable-version`, `unreadable-manifest`, `unreadable-sibling-manifest`, `unreadable-changelog`, `downgrade`, `not-an-upgrade` or `bad-date`, with nothing written and exit 1. | accurate | accurate | 2 |
| MILESTONE-07 | cadence-core/workflows/milestone.md | 77-82 | A `siblings[]` entry with `action:"refuse"` leaves top-level `ok` true, and means a sibling that was READABLE but not upgradeable - an unreadable one refuses the whole run instead. | accurate | accurate | 2 |
