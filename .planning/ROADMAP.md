...
- [x] **Phase 2: One reader for the lease grammar** - `plan-overlap` and `lease-check` resolve a declared path through one module, so the pre-flight gate cannot admit a pair the enforcement would refuse
