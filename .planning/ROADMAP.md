...
- [x] **Phase 33: Execution and verification under the boundary** - a worker gets a dispatch id, reports progress and completion as typed pieces, the binary renders SUMMARY.md, and review material reaches the binary without crossing the wire
...
- [ ] **Phase 38: The suite is a gate, not a guillotine** - a red suite raises a repair checkpoint the owner answers, one repair and one rerun on the record, and a dispatch keeps the prompt it was issued with; the two rules decided under 3.7.12 that blocked seven of sixteen native plans are re-decided with that evidence
...
**Goal.** Two execution-model rules were decided under 3.7.12 before the
binary had executed a plan. D-112 and acceptance.md rule 5 say the suite
runs once per plan, so a red suite ends the plan and D-120 makes the only
repair a new gap plan; sixteen native plans have run and seven went red at
that line, each one a planning cycle, twice with the fix already in the
executor's hands. Phase 6 made a dispatch's identity its prompt byte count
under the current renderer, so any change to shared instruction text breaks
every pinned rendering in files no lease names and refuses every dispatch
issued before it. D-163: a failing launch ends the launch, not the plan;
the binary raises a repair checkpoint naming the failing tests and the
files, the owner's answer allows exactly one repair and one further launch,
everything is retained, and a second red blocks as today. D-164: gap plans
are for a refused repair, a second red or a redesign, never a first red;
an out-of-lease repair is retained as a deviation. D-165: a dispatch keeps
the prompt it was issued with and answers with it; nothing re-renders, and
a rebuilt binary never refuses an open plan. D-166: files the binary
renders are regenerated by the binary and sit inside every lease by rule.
D-167: `execute-next` takes a plan number. D-168: a suite receipt always
carries the failing test names. D-169, phase-local: the plans land by hand
