...
`v1.3.0` is shipped and tagged — the latest in the lineage from `v1.0.0`
through `v1.1.0`, `v1.2.0` (cross-model review seam, durable-decision recall,
DeepSeek provider), the `v1.2.1` sweep-highs patch, and the `v1.3.0` liteSpeed
flow-and-latency pass. Git history and each release tag are that cycle's archive.

**v1.3.1 — Tech debt** is the active cycle (opened 2026-07-24): collect every
