...
- [x] **Phase 2: References load where they are used** - split `git.md`, extract the triage gate and restate it as a multi-select, resolve the `conventions.md` phantoms, and judge every remaining eager include per skill
- [x] **Phase 3: The surfaces that are always on, and the ratchet that watches them** - trim the descriptions riding every session everywhere, then put `references/` and `templates/` under the weight budget
