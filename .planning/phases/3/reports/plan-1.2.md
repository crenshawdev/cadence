...
| 6: route.mjs reads no table | NOT COMMITTED | Written in full and green on every part of `Verify:` EXCEPT the whole-suite line. See the deviation below. |
Deviations:
...
`node cadence-core/bin/test.mjs` passes". It does not, and cannot inside this
plan's lease. The Action requires deleting the `bad-table` failure from
`route.mjs`; `cadence-core/bin/reason-census.test.mjs:104` carries `'bad-table'`
in its hand-maintained `REASON_TOKENS` list, and its own refusal message says
the fix is "it was deliberately removed, in which case delete its entry here in
the same commit". Two cases fail on it - `every committed refusal token is still
produced by the live tree` and `the census is ONE-DIRECTIONAL: a token the list
does not carry passes` - and they are the ONLY two red in the tree
(3809 of 3811 pass). MEASURED, not inferred: deleting `'bad-table', ` from that
line takes the suite to 3811/3811, and staging the six task-6 files with it
returns
`{"ok":false,"reason":"undeclared-files",...,"undeclared":["cadence-core/bin/reason-census.test.mjs"]}`
from `planning.mjs lease-check --phase 3 --plan 1`. The probe edit has been
reverted, so the working tree carries in-lease changes only. Stopped rather than
committing a task whose stated verification fails.
...
Tasks 1-5 are committed (`34de9d68`, `b275c82a`, `6d55fdfc`, `1f4436b9`,
`b9e2f86f`). Task 6 is written, UNCOMMITTED, and passes every part of its
`Verify:` except the whole-suite line. Modified and unstaged:

- `cadence-core/bin/route.mjs` - `TABLE`, `TABLE_PATH`, the
  `CADENCE_ROUTE_TABLE` seam, the `bad-table` failure and the `table` subcommand
  deleted; `riskCategories` is now
  `schemaValues('review.triggers.risk_surface.surfaces')`; the usage detail is
  `subcommand: resolve`; the header's `table` line and every `route-table`
  mention are gone. `git grep -n "route-table\|CADENCE_ROUTE_TABLE\|\bTABLE\b"`
  over the file prints nothing.
- `cadence-core/bin/route.test.mjs` - 147/147 pass. `opts.table` plumbing gone
  from the `resolve` helper; the `#40` section re-keyed onto `config.schema.json`
  (absent AND malformed both answer `bad-schema`, hint naming the file); the
  `EXP-01` gate section re-keyed onto `CADENCE_CONFIG_SCHEMA` (a hostile
  `risk_surface` gate default is ignored without the sentinel and takes with
  it); `SHIPPED_TABLE` deleted and the frontmatter-agreement walk re-keyed onto
  `RUNG_FILES` (30 stems, up from the 19 the cells reached); the two `table`
  dump tests deleted.
- `cadence-core/bin/lib/arg-contract.mjs` - the `table` row under
  `CONTRACTS['route.mjs']` deleted.
- `cadence-core/bin/lib/test-seam.mjs` - the header names
  `CADENCE_CONFIG_SCHEMA` and `CADENCE_PLUGIN_MANIFEST` as the two redirected
  ground-truth files, and the not-a-privilege-boundary paragraph names
  `CADENCE_CONFIG_SCHEMA`.
- `cadence-core/bin/trace-suggest.test.mjs` - one assertion Task 5 missed:
  `SGT-01: R3 proposes the rung the escalated resolves actually landed on` still
  matched `/shipped/` on the unset sentence. Now matches
  `/the schema default decides it/`.

`npx tsc -p tsconfig.ci.json` is clean and `node cadence-core/bin/self-verify.mjs`
reports `ok:true` with an empty `problems` list. With
`cadence-core/route-table.json` moved out of the tree,
`route.mjs resolve --role cad-executor` still returns `ok:true` - the file is
unread, and PLAN-2 deletes it.

## What unblocks it

Add `cadence-core/bin/reason-census.test.mjs` to PLAN-1's `files:` lease, and
the edit is one token at line 104 -
...
which is what that file's own failure message instructs and what the probe
measured green. No other plan of this phase declares the file, so the overlap
costs nothing.

The alternative, which was rejected: keep a `fail('bad-table', ...)` arm in
`route.mjs` for a table it no longer reads, so the census still finds the
literal. That is the control-reports-success shape this repo closes rather than
opens, and the Action explicitly says delete the failure.
