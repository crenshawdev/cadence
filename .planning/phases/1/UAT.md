...
reported: behavior wrong - the torn-layer arm is anchored to the repo layer's path, so a torn user-global layer produces neither ask nor deny
severity: major
...
reported: behavior wrong - the staged-set read misses renames and mangles non-ASCII paths
severity: blocker
...
reported: behavior wrong - the gate reads the whole staged index, so a file the USER pre-staged hard-blocks the phase
severity: major
...
reported: behavior wrong - pnpm-lock.yaml, named in the item, still matches the concurrency surface
severity: major
...
reported: unwired - the drop-outs most likely to fire throw before the trace event's `meta` exists, so nothing is written
severity: major
...
reported: behavior wrong - the comparison is against the highest semver tag rather than tag membership, so an untagged maintenance milestone is refused with a reason asserting a publication that never happened
severity: major
