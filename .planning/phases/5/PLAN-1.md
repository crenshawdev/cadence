...
`Decision::Gate`, `DecisionRecord` and `Snapshot.data` already provide durable capacity; `Session::request` wraps replacements under `current`, while `commit_derivation` preserves full data (`crates/cadence/src/import/mod.rs:404`, `:422`).
