...
- **EVD-01**: A phase's joined run record has a publishable form. `planning.mjs trace export` reads `.planning/trace.jsonl` and emits a redacted artifact under a STATED rule - what is dropped, what is preserved, and why the raw file stays gitignored - where a field the rule does not name is dropped rather than emitted, so a future event family cannot leak by default. The raw record keeps its ignore line; publication is an export, never a lifting of the ignore, on the same reasoning D-01 applied to the capture queue in phase 1
- **EVD-02**: What Cadence does at runtime is a committed artifact rather than a claim about the source: an exported trace, the `weight.mjs` resident and turn-one byte figures, the commit range, and the environment it ran in (plugin version, Node version, host), plus the exact command that regenerates the measurable half. The trace half records a run that happened and is not reproducible by design - the artifact states that rather than implying otherwise. **The trace comes from a project that is NOT Cadence.** Cadence measuring Cadence is the weakest available evidence for the one thing the dossier could not check, and it is unnecessary: Cadence runs on nine other projects, five of them tagged and shipped (`tempest` v2.9.5, `burnrate` v1.0.3, `weathervane` v0.9.1, `atmos` v1.3.1-alpha, `placer` v0.1.0-beta.2), with `hindsight` (5 of 7) and `assistant` (4 of 8) mid-cycle. Stated prerequisite, not a free swap: no non-Cadence project has a trace today because none has run a phase since `QW-02` shipped in `v2.5.0`, so this requirement waits on the next phase run in one of the live projects and names which one it took. Added 2026-08-08 from the 2026 harness dossier, which audited Cadence at `v2.4.0`, ranked it #1 of 35, and capped Caching (12.78% effective weight), Context (12.92%) and Security (7.30%) on the identical reason: "direct runtime proof for this submechanism remains incomplete", with "operationally verified: not run in this environment". Those three are the only categories whose stated blocker is evidence rather than a defect, so no amount of code closes them
