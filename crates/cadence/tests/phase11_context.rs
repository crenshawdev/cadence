...
fn fixture(native: bool, phase: bool, pending: bool) -> tempfile::TempDir {
    ...
    if phase {
        ...
        fs::write(
            root.join("phases/11/CONTEXT.md"),
            "# Prior context\n\nOwner prose: déjà vu.\n",
        )
        .unwrap();
    }
    ...
    for (native, phase, pending) in [
        ...
        for intake_only in [true, false] {
            ...
            let intake = client.call(
                "cadence_query",
                json!({"operation":"context-intake","phase":11}),
            );
            assert_eq!(
                intake["status"], "ok",
                "context intake must be supported: {intake}"
            );
            ...
            if !intake_only {
                for approval in [
                    ...
                    if approval["approved"] == true {
                        ...
                        assert_eq!(
                            answer["status"], "ok",
                            "valid draft must reach context authoring: {answer}"
                        );
            ...
            assert_eq!(
                tree(temp.path()),
                before,
                "read-only reopen after server exit"
            );
        }
    }
}
...
fn initialized_fixture(phase_directory: bool) -> tempfile::TempDir {
    ...
    tokio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        store
            .request(cadence::store::writer::Operation::Transact(
                cadence::store::transaction::Transaction {
                    ...
                    decisions: vec![model::DecisionRecord {
                        version: 1,
                        id: "prior-decision".into(),
                        revision: 1,
                        origin: model::Origin {
                            source: "owner".into(),
                            original: model::Evidence::Missing,
                        },
                        decision: model::Decision::Gate {
...
fn phase11_approved_context_persists_truths_and_decisions() {
    let examples = [
        (
            "the owner opens the context",
            "sees",
            "the approved decisions",
            "literal",
            "When the owner opens the context, the owner sees the approved decisions.",
        ),
        (
            "the owner requests a receipt",
            "gets",
            "receipt 42",
            "property",
            "When the owner requests a receipt, the owner gets receipt 42.",
        ),
        (
            "the owner changes an approved set",
            "is refused",
            "a revision",
            "literal",
            "When the owner changes an approved set, the owner is refused a revision.",
        ),
        (
            "the owner reads café notes",
            "sees",
            "日本語 prose",
            "property",
            "When the owner reads café notes, the owner sees 日本語 prose.",
        ),
        (
            ...
            "three entries",
            "literal",
            "When the owner requests a list, the owner gets three entries.",
        ),
        (
            "the owner submits an empty name",
            "is refused",
            "approval",
            "property",
            "When the owner submits an empty name, the owner is refused approval.",
        ),
        (
            "the owner opens the final page",
            "sees",
            "the last decision",
            "literal",
            "When the owner opens the final page, the owner sees the last decision.",
        ),
    ];
    for phase_directory in [false, true] {
        for count in 1..=7 {
            ...
            assert_eq!(
                answer["status"], "ok",
                "approved context must publish: {answer}"
            );
            ...
            assert!(
                markdown.contains("## Flagged assumptions\n\n- The reader understands 日本語.\n")
            );
            let truth_section = markdown
                .split("## Truths\n\n")
                .nth(1)
                .unwrap()
                .split("\n## ")
                ...
                .unwrap()
                .trim_end();
            let expected_lines = examples[..count]
                .iter()
                .enumerate()
                .map(|(i, example)| format!("- T{}. {}", i + 1, example.4))
                .collect::<Vec<_>>()
                .join("\n");
            ...
            assert_eq!(
                reopened.snapshot.data["unrelated"],
                json!({"keep":"unchanged"})
            );
            ...
            for (i, example) in examples[..count].iter().enumerate() {
                assert_eq!(
                    saved["truths"][i],
                    json!({"id":format!("T{}",i+1),"phase":11,"version":1,
            ...
            let prior_snapshot = Snapshot::parse(
                ...
                before[Path::new(".planning/decisions.jsonl")]
                    .as_ref()
                    .unwrap(),
            )
            .unwrap();
            for (key, value) in prior_snapshot.data.as_object().unwrap() {
                assert_eq!(
                    &reopened.snapshot.data[key], value,
                    "unrelated snapshot field {key}"
                );
            }
            ...
            expected_tree.insert(
                PathBuf::from(".planning/phases/11/CONTEXT.md"),
                Some(markdown.into_bytes()),
            );
            expected_tree.insert(
                PathBuf::from(".planning/state.json"),
                after[Path::new(".planning/state.json")].clone(),
            );
            assert_eq!(
                after, expected_tree,
                "no pending intent or temporary files after acknowledgment"
            );
        }
    }
}
...
fn phase11_sentence_fault_names_rule_and_slot() {
    let mut faults = vec![
        (
            "trigger",
            Some(json!("the owner opens or closes the context")),
            "one-trigger",
        ),
        (
            "observer",
            Some(json!("the owner and the caller")),
            "one-observer",
        ),
        (
            "observer",
            Some(json!("the owner & the caller")),
            "one-observer",
        ),
        (
            "observer",
    ...
    for (slot, value, rule) in faults {
        for approved in [false, true] {
            ...
            if let Some(value) = value.clone() {
                request["submission"]["truths"][0][slot] = value;
            } else {
                request["submission"]["truths"][0]
                    .as_object_mut()
                    .unwrap()
                    .remove(slot);
            }
    ...
    for position in [0, 1, 2] {
        for attestation in [None, Some(false)] {
            for approved in [false, true] {
                ...
                truths[position]["outcome"] =
                    json!("ContextState.approved equals true inside the writer struct");
                if let Some(value) = attestation {
                    truths[position]["observable"] = json!(value);
                } else {
                    truths[position]
                        .as_object_mut()
                        .unwrap()
                ...
                assert_eq!(
                    control["status"], "ok",
                    "owner attestation controls this decision: {control}"
                );
    ...
    for position in [0, 1, 2] {
        for attestation in [None, Some(false)] {
            for approved in [false, true] {
                ...
                truths[position]["outcome"] =
                    json!("the expected answer supplied by a fresh model prose call");
                if let Some(value) = attestation {
                    truths[position]["fixed_oracle"] = json!(value);
                } else {
                    truths[position]
                        .as_object_mut()
                        .unwrap()
                ...
                assert_eq!(
                    control["status"], "ok",
                    "owner attestation controls the oracle: {control}"
                );
                assert_eq!(control["persisted"], false);
                client.finish();
                assert_eq!(tree(temp.path()), before);
...
fn phase11_eighth_truth_requires_phase_split() {
    for approved in [false, true] {
        let temp = initialized_fixture(true);
        fs::write(
            temp.path().join(".planning/phases/11/CONTEXT.md"),
            "# Prior context\nKeep this text.\n",
        )
        .unwrap();
        let before = tree(temp.path());
...
fn phase11_identity_collision_is_refused() {
    for (source, destination, slot, index, id) in [
        ...
        for approved in [false, true] {
            ...
            if source == destination {
                ...
                request["submission"][destination]
                    .as_array_mut()
                    .unwrap()
                    .push(repeated);
            } else {
            ...
            let answer = client.call(
                "cadence_apply",
                if approved { approve(request) } else { request },
            );
    ...
    assert_eq!(
        first["status"], "ok",
        "T1 and D-01 share a suffix but are distinct full IDs: {first}"
    );
    ...
    for (family, id) in [
        ...
        for approved in [false, true] {
            ...
            request["submission"]["scope"] =
                json!("A different submission after the first approval.");
    ...
    assert_eq!(
        retry["status"], "ok",
        "an exact durable receipt is a retry: {retry}"
    );
    ...
    assert_eq!(
        tree(temp.path()),
        winner,
        "exact retry must not create another approval"
    );
    ...
    assert_eq!(
        answer["status"], "ok",
        "full identities are phase-scoped: {answer}"
    );
    client.finish();
    let after = tree(temp.path());
    ...
    assert_eq!(
        after, allowed,
        "phase 11 winner and all unrelated files survive phase 12"
    );
    let data: Value =
        serde_json::from_slice(&fs::read(temp.path().join(".planning/state.json")).unwrap())
            .unwrap();
    assert_eq!(
        data["data"]["context"]["phases"]["12"]["truths"][0]["id"],
        "T1"
    );
    assert_eq!(
        data["data"]["context"]["phases"]["11"]["truths"][0]["id"],
        "T1"
    );
}
