...
async fn dispatch_fixture(store: &Store) -> (View, ActiveDispatch) {
    ...
    let mut candidate = build_dispatch(&plan, &set, 0, &"b".repeat(40)).unwrap();
    candidate.prompt = "x".into();
    candidate.prompt_digest = cadence::store::model::digest(candidate.prompt.as_bytes());
