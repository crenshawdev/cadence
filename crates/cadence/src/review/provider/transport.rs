...
pub async fn request(transport: &dyn Transport, request: Request, timeout: Duration) -> Result<Acquired, String> {
    tokio::time::timeout(timeout, async {
        ...
        Ok(Acquired { status: response.status, headers: response.headers, raw, json })
    }).await
        .map_err(|_| format!("request timed out after {}ms", timeout.as_millis()))?
        .map_err(|error| diagnostics::excerpt(&error))
}
