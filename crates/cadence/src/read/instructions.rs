//! The one compiled read contract shared by every Cadence caller.

/// This paragraph leads server initialization so a host can discover the
/// boundary before loading any Cadence skill.
pub const CONTRACT: &str = r#"Cadence is the only project read surface. Use `cadence_query` with `search` to find source, `read` only with a location or file reference Cadence issued (or a named unit under that reference), and `document` with a process identity to inspect contexts, plans, roadmap rows and task summaries. Never open a project file with a host file tool, shell command, standalone excerpt server, or a path/range invented by the caller.
