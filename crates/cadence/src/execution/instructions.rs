...
close, or the close is refused with the path named."#;

/// The branch rule every executor scope keeps.
const BRANCH_RULE: &str = r#"Work on the current branch;
do not push, reset, amend, revert or force-push."#;

/// The task scope's protocol: the same executor, no dispatch and no lease,
/// with the protected-branch and risk dispositions the binary applies at
/// task-open and task-close (D-209).
const TASK_PROTOCOL: &str = r#"## Task protocol
...
/// Which executor is being composed from the one role source: the phase
/// executor under a dispatch and its lease, or the task executor with the
/// lease disabled and the branch and risk dispositions kept (D-209).
