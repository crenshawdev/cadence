...
So model is a live lever and effort is not. Effort still varies, it is just paid for in files: a rung is an agent file whose frontmatter carries that effort, and dispatching a role harder means dispatching a different file. `cad-plan-checker-medium` and `cad-plan-checker-high` are the same contract at two depths, and 19 files cover the six roles. `cadence-core/bin/lib/rung-agent.mjs` says which file carries which rung, stated per role rather than derived, because no naming convention is true of all 19 (the assumptions-analyzer's unsuffixed file is its `xhigh` rung and the `-high` sibling is the lower one). Every rung costs a file, and every registered agent's name and description ride in your main session prompt, so the reachable rungs are exactly the ones some routing cell names. Two things keep the price honest: CI refuses a rung a cell names with no file, and the stale reverse, a rung file no cell reaches; and it refuses a rung file that carries any instruction of its own, because the contract lives exactly once, in the skill every rung file of that role preloads.

If you don't want to think about any of that, there is one question with three answers: what does a break in this project cost? `solo` means nobody else runs it and a break costs only your time, `shipped` means other people run it and a break comes back as a bug report, `critical` means a break is not a bug report. Your answer picks a row of a small table, and the role picks the cell in it. A cell is not a model, it is the whole quality bundle: which model, which rung to start at, which rung a failed attempt climbs to, which review gates fire, and whether the deep verify pass runs. Quality is not one dial, and no amount of effort expresses "fire a blocking cross-model review before this ships". The models are Claude aliases rather than dated ids, so the table never rots; the routed vocabulary is `sonnet` and `opus`, and `haiku` and `fable` are reachable only by an explicit `model.overrides` pin, which is your assertion about your own org rather than a rung on this ladder. The guardrails keep it honest: your explicit pick always wins, a gate you set beats the level's and says so, and a failed attempt that climbs to a harder rung records why in its reason instead of doing it behind your back. CI refuses a retry rung that sits below the rung it started on, so a retry can never think less while reporting that it thought more.
