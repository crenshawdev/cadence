...
So model is a live lever and effort is not. Cadence sets effort per role, where it belongs. The planner, verifier, reviewer, and assumptions-analyzer run high or xhigh because that is what those jobs need, and the plan-checker runs low because it is a cheap gate. When that cheap gate fails an attempt and gets re-dispatched, it does the one thing SKILL.md actually allows, it swaps to a sibling agent file that carries the higher effort in its own frontmatter, `cad-plan-checker-high`. Every rung a role can be dispatched at is a file like that, and `cadence-core/route-table.json` declares which rungs each role reaches, as data, per role rather than as a full ladder-by-roles cross product, because every rung listed costs a file and every registered agent's name and description ride in your main session prompt. So effort is a dial the routing layer can turn, paid for in files rather than pretended into existence. Two things keep the price honest: CI refuses a rung the table names and the tree lacks (and the stale reverse, a rung file no role declares), and it refuses a rung file that carries any instruction of its own, because the contract lives exactly once, in the skill every rung file of that role preloads.

If you don't want to think about any of that, there is one question with three answers: what does a break in this project cost? `solo` means nobody else runs it and a break costs only your time, `shipped` means other people run it and a break comes back as a bug report, `critical` means a break is not a bug report. Each answer maps an agent's tier (light, standard, heavy) to a Claude alias (haiku, sonnet, opus, fable). Aliases, not dated model ids, so the table never rots. The guardrails keep it honest: role tiers act as floors so a formatter never gets promoted to opus, your explicit pick always wins, and a failed attempt that re-dispatches at a harder rung records why in its reason instead of doing it behind your back.
...
Read the code: `cadence-core/bin/route.mjs` (the resolver), `cadence-core/route-table.json` (roles, the stakes matrix, and the per-role rung ladder, all editable data, no code change to retune), `cadence-core/bin/route.test.mjs`. Design record: `DESIGN.md`, "Model routing."
