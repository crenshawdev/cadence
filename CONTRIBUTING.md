...
Run `cargo nextest run -p cadence --bin cadence instruction_lint` for the native instruction lint. The existing workspace CI in `.github/workflows/test.yml` runs it with the workspace suite.
