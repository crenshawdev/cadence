...
| `skills/cad-health/SKILL.md:103-109` | The maintainer-facing cross-reference ("Membership is deliberate and must not drift back to 'sorts above the newest tag'... the two surfaces have to answer the same question about the same repo") and the `v2.4.0` anecdote | `cadence-core/bin/lib/branch-decision.mjs`'s header, which already carried the membership-not-sort-order reasoning and this module's single-reader discipline. The block SPLIT: the `v1.9.1` worked example STAYS eager, because it decides a verdict the model issues at runtime and removing it makes `/cad-health` report a legitimate untagged maintenance milestone as drift - the regression the paragraph was written after (phase 2 D-20). |
| `workflows/plan.md:8-9` | The design-history contrast: "One planner plus one optional checker, not a four-agent fan-out; 4 flags, not ~20; one bounded revision, not a convergence loop." | Nowhere, deliberately: all three describe a design Cadence has never shipped, so the contrast informs no runtime decision. The routing rules beside it stay eager (research is /cad-context's job, second opinions belong to the review subsystem), as does the pipeline's commit-docs terminal step, which the skill's objective does not carry. `.planning/DOCS-CLAIMS.md` row PLAN-03 is retired with it. |
| (not cut) `workflows/plan.md:330-335` | "Both re-dispatches close on their own, at their own step..." | STAYS eager. The sweep listed it as duplicated verbatim in `cadence-core/bin/trace.test.mjs`; that row is FALSE - `grep -n 'Both re-dispatches\|SOME terminal' cadence-core/bin/trace.test.mjs` returns nothing, the test's `BRACKETING` map pins counts and not prose, and the paragraph carries a rule the model applies at runtime (close BOTH re-dispatch brackets at their own step). |
| `workflows/verify.md:2-11` | The `<purpose>` block's restatement of `skills/cad-verify/SKILL.md`'s `<objective>`: the one-item-at-a-time walk, the persistent `.planning/phases/<N>/UAT.md`, survival across `/clear`, resume at the first untested item, and the no-auto-fixer routing | Nowhere, deliberately: the objective states all five on the same turn-one context. What the objective does NOT state stays eager - the severity-inference rule, the seam-ownership paragraph and the two cold branches. |
| `workflows/verify.md:84-89` | The `fields_version` legacy archaeology: that `uat init` writes the marker before it looks at an item, so no seam-written checklist can present as legacy, and that legacy now also requires a CONTEXT declaring no ids | `cadence-core/bin/planning.mjs`, the `fieldless`/legacy block that decides it, which already carried the argument in full. Only a pointer was added there, saying this is now the tree's ONLY statement of it. The additive runtime consequence (`untraced`) stays eager. |
